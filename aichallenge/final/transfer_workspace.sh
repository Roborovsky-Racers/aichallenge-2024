...
ssh -t "$AIC_HOSTNAME" "bash -c 'cd $AIC_REMOTE_DIR/aichallenge/; mkdir -p backup/temp; if [ -d workspace/src ]; then tar zcvf backup/temp/src_$DATE.tar.gz workspace/src && rm -rf workspace/src; else echo \"workspace/src does not exist\"; fi'" || true
